...
export default {
  ...
  setup() {
    let time: Ref<Date> = ref(new Date(0)); // contains Date object with time 00:00:00
    let timer: Ref<number> = ref(0); // containing timer setInterval
    let seconds: Ref<string> = ref('00'); // for seconds to display at page
    let minutes: Ref<string> = ref('00'); // for minutes to display at page
    let isRunning: Ref<boolean> = ref(false); // to check if timer is stoped
